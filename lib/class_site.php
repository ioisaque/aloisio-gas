<?php
  ...
  class Site
  {
      ...
      public function ProcessarPedido()
      {
		  ...
		  $bairro = post('bairro');
		  $cidade = post('cidade');
		  $pagamento = post('id_pagamento'); 		  
			
		  $array_produtos = post('id_produto');
		  $array_quantidade = post('quantidade');
		  $contar_produtos = count($array_produtos);	
		  
		  for($i = 0;$i < $contar_produtos;$i++) {	  
			  if ( $array_quantidade[$i] > 0 )
				$produto = true;
		  }

		  if (!$produto)
				Filter::$msgs['produto'] = lang('MSG_ERRO_PRODUTO');
		  
		  if (empty($nome))
				Filter::$msgs['nome'] = lang('MSG_ERRO_NOME');	
			
		  if (empty($telefone))
				Filter::$msgs['telefone'] = lang('MSG_ERRO_TEL');	
		  ...
		  if (empty($pagamento))
				Filter::$msgs['pagamento'] = lang('MSG_ERRO_FORMA_PAG');	
          ...
          if (empty(Filter::$msgs)) {
			...
			for($i = 0;$i < $contar_produtos;$i++) {	
				$id_produto = $array_produtos[$i];
				$quantidade = $array_quantidade[$i];
				if($quantidade > 0) {
					...
					$data = array(
					  ...
					  $id_venda = self::$db->insert("vendas", $data);
					  ...
					  if($controle == 2){
						...
						self::$db->update("cliente", $data_cliente, "id=".$id_cliente);
					}
					...
					$id_pagamento = self::$db->insert("pagamentos", $data);
					...
					if ($tipo == 1) { 
						...
						$id_controle = self::$db->insert("pag_controle", $data_pag);
              ...
              if (self::$db->affected()) {
                  Filter::msgOk("PEDIDO ENVIADO COM SUCESSO!!");               
       ...
       * @return
       */
	  public function getGanhadores($l = false)
      {
          $limit = ($l) ? " LIMIT 3 " : "";
		  $sql = "SELECT g.* " 
		  . "\n FROM ganhadores as g "
		  . "\n ORDER BY STR_TO_DATE(data_sorteio,'%d/%m/%Y') DESC $limit ";
          $row = self::$db->fetch_all($sql);
